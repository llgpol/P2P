package com.yueqian.business.util;
...
public class CalculatetUtil {
	...
	public static BigDecimal getMonthlyRate(BigDecimal yearRate) {
		...
		return yearRate.divide(ONE_HUNDRED).divide(NUMBER_MONTHS_OF_YEAR, BigConst.CAL_SCALE, RoundingMode.HALF_UP);
	}
	...
	public static BigDecimal calTotalInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate, int monthes2Return) {
		...
		if (returnType == BigConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				totalInterest = bidRequestAmount.multiply(monthlyRate).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			} else {
				...
				// 算出每月还款
				BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(temp3, BigConst.CAL_SCALE, RoundingMode.HALF_UP);
				// 算出总还款
		...
		} else if (returnType == BigConst.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
		...
		return DecimalFormatUtil.formatBigDecimal(totalInterest, BigConst.STORE_SCALE);
	}
	...
	public static BigDecimal calMonthlyInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate, int monthIndex, int monthes2Return) {
		...
		if (returnType == BigConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			} else {
				BigDecimal temp1 = bidRequestAmount.multiply(monthlyRate);
				BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return);
				BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return).subtract(BigDecimal.ONE);
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = (temp1.subtract(monthToReturnMoney)).multiply(temp4).add(monthToReturnMoney).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
					...
					for (int i = 1; i < monthes2Return; i++) {
						...
						monthlyInterest = (temp1.subtract(monthToReturnMoney)).multiply(temp5).add(monthToReturnMoney).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
	...
	public static BigDecimal calMonthToReturnMoney(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate, int monthIndex, int monthes2Return) {
		...
		if (returnType == BigConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			if (monthes2Return == 1) {
				monthToReturnMoney = bidRequestAmount.add(bidRequestAmount.multiply(monthlyRate)).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			} else {
				BigDecimal temp1 = bidRequestAmount.multiply(monthlyRate);
				BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return);
				BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return).subtract(BigDecimal.ONE);
				// 算出每月还款
				monthToReturnMoney = temp1.multiply(temp2).divide(temp3, BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			}
		} else if (returnType == BigConst.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
			BigDecimal monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			if (monthIndex == monthes2Return) {
				monthToReturnMoney = bidRequestAmount.add(monthlyInterest).setScale(BigConst.CAL_SCALE, RoundingMode.HALF_UP);
			} else if (monthIndex < monthes2Return) {
				monthToReturnMoney = monthlyInterest;
			}
		}
		return DecimalFormatUtil.formatBigDecimal(monthToReturnMoney, BigConst.STORE_SCALE);
	}
	...
	public static BigDecimal calBidInterest(BigDecimal bidRequestAmount, int monthes2Return, BigDecimal yearRate, int returnType, BigDecimal acturalBidAmount) {
		...
		// 所占比例
		BigDecimal proportion = acturalBidAmount.divide(bidRequestAmount, BigConst.CAL_SCALE, RoundingMode.HALF_UP);
		BigDecimal bidInterest = totalInterest.multiply(proportion);
		return DecimalFormatUtil.formatBigDecimal(bidInterest, BigConst.STORE_SCALE);
	}
...
//		return DecimalFormatUtil.formatBigDecimal(amount.multiply(BigConst.RECHARGE_FEE_RATE), BigConst.STORE_SCALE);
//	}
